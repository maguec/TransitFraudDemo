    ...
    for i in range(1350):
        ...
        for j in range(30):
            writer.writerow({
                ...
                ).strftime("%Y-%m-%d") + "T{:02}:{:02}:{:02}.0Z".format(morning_hour, fake.random_int(min=0, max=20)+half_hour*30, fake.random_int(min=0, max=59)),
            ...
            writer.writerow({
                ...
                'ride_date': fake.date_time_between_dates(
                    datetime_start=datetime.now() - relativedelta(days=j),
                    datetime_end=datetime.now() - relativedelta(days=j - 1),
                ).strftime("%Y-%m-%d") + "T{:02}:{:02}:{:02}.0Z".format(evening_hour, fake.random_int(min=0, max=20)+half_hour*30, fake.random_int(min=0, max=59)),
